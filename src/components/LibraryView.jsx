...
import { isTV, STATUSES, PLATFORMS, titleOf } from '../tmdb.js'
...
export function LibraryView({ lib, profiles, settings }) {
  ...
  const shows = lib.library.filter(isTV)
  ...
  const filtered = pool
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'rating') {
        const avgA = profiles.profiles.length
          ? Object.values(a.ratings || {}).reduce((s, v) => s + v, 0) / (Object.values(a.ratings || {}).length || 1)
          : 0
        const avgB = profiles.profiles.length
          ? Object.values(b.ratings || {}).reduce((s, v) => s + v, 0) / (Object.values(b.ratings || {}).length || 1)
          : 0
        return avgB - avgA
      }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', marginBottom: 20 }}>
        {[{ id: 'tv', label: 'Shows', count: shows.length }, { id: 'movie', label: 'Films', count: films.length }].map(t => (
          <button key={t.id} onClick={() => setMediaTab(t.id)} style={{
            ...
            cursor: 'pointer', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: -1,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(135px, 1fr))', gap: 14 }}>
            {filtered.map(item => (
              <PosterCard key={item.id} item={item}
                ...
                badge={item.status} badgeColor={STATUS_COLORS[item.status]} />
            ))}
          </div>
        )
      }
