...
export function DiscoverView({ lib }) {
  ...
  const pool = query
    ? results
    : browseTab === 'trending' ? trending : topRated

  const display = typeFilter === 'all'
    ? pool
    : pool.filter(r => r.media_type === typeFilter)

  return (
    <div>
      <PageHeader
        title="Discover"
        sub="Find something worth losing sleep over."
      />
      ...
      <div style={{ position: 'relative', marginBottom: 18 }}>
        <span style={{
          ...
          color: 'var(--text-low)', fontSize: 17, pointerEvents: 'none',
        }}>⌕</span>
        <input
          value={query}
        ...
        {loading && (
          <span style={{
            ...
            color: 'var(--text-low)', fontSize: 11,
          }}>…</span>
        )}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {!query && BROWSE_TABS.map(t => (
          <Pill key={t.id} active={browseTab === t.id} onClick={() => setBrowseTab(t.id)}>
            {t.label}
          </Pill>
        ))}
          ...
          <div style={{
            ...
            {display.slice(0, 40).map(item => {
              ...
              const badgeColor = entry
                ? ({ 'Watching': '#c8922a', 'Completed': '#4a7a4a', 'On Hold': '#7a6a3a', 'Dropped': '#8a3a3a', 'Plan to Watch': '#3a5a7a' })[entry.status]
                : inWl ? '#3a5a7a' : null
              return (
                <PosterCard
