...
const NAV = [
  ...
  },
  {
    ...
    icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>,
  },
  {
    id: 'stats', label: 'The Record',
...
export default function App() {
  ...
  const props = { lib, profiles, settings }

  const VIEWS = {
    discover:  <DiscoverView  {...props} />,
    library:   <LibraryView   {...props} />,
    watchlist: <WatchlistView {...props} />,
    stats:     <StatsView     lib={lib} profiles={profiles} />,
    settings:  <SettingsView  profiles={profiles} settings={settings} />,
  }
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '0 24px 32px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            ...
            <div style={{ fontSize: 16, fontFamily: "'Playfair Display', serif", fontWeight: 900, color: 'var(--text-hi)', letterSpacing: '-0.01em', lineHeight: 1 }}>WatchSkull</div>
          </div>
          <div style={{ fontSize: 9, color: 'var(--text-faint)', letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 7, paddingLeft: 31 }}>
            dead serious about what you watch
          </div>
          {/* Household member avatars */}
          {profiles.profiles.length > 0 && (
            <div style={{ display: 'flex', gap: 6, marginTop: 14, paddingLeft: 31, flexWrap: 'wrap' }}>
              {profiles.profiles.map(p => (
                <span key={p.id} title={p.name} style={{ fontSize: 18, cursor: 'default' }}>{p.emoji}</span>
              ))}
            </div>
          )}
        </div>
          ...
          {NAV.map(n => {
            ...
            return (
              <button key={n.id} onClick={() => setView(n.id)} style={{
                width: '100%', background: active ? 'var(--bg-hover)' : 'none',
              ...
              >
                ...
                {n.label}
                {badge > 0 && (
                  <span style={{ marginLeft: 'auto', background: 'var(--amber)', color: '#0a0806', fontSize: 9, fontWeight: 800, padding: '1px 6px', borderRadius: 10, lineHeight: 1.6 }}>{badge}</span>
                )}
              </button>
            )
          })}
      ...
      {/* Main */}
      <main style={{ flex: 1, padding: '40px 44px', maxWidth: 1140, overflowY: 'auto' }}>
        {VIEWS[view]}
      </main>
    </div>
  )
